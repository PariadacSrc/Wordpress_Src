...
$recipe_post_data = array(
  ...
  'recipe_rating'     => 0,
  'recipe_cook_time'  => '?',
  'recipe_cook_time_text' => esc_html__('min', 'goza'),
  'course_term_list'         => get_the_term_list( get_the_ID(), 'course', '', ' ' ),
);

/* check featured image exist */
if ( has_post_thumbnail() ) {
    $featured_image_data = get_the_post_thumbnail_url(get_the_ID(), 'goza-image-medium');
...
<article id="post-recipe-<?php the_ID(); ?>" <?php post_class( implode(' ', $article_classes) ); ?> itemscope="itemscope" itemtype="http://schema.org/RecipePosting" itemprop="recipePost">
    ...
    <div class="featured-image-wrap">
      <div href="<?php the_permalink(); ?>">
        ...
        <a class="read-more-link" href="<?php the_permalink() ?>"><?php echo esc_html__('Read More', 'goza') ?> <i class="ion-ios-arrow-thin-right"></i></a>
      </div>
      ...
      <div class="extra-meta">
        <div class="meta-item cook-time">
          <i class="fa fa-clock-o" aria-hidden="true" data-toggle="tooltip" data-placement="bottom" title="<?php echo esc_html__('cook time', 'goza'); ?>"></i> <?php echo "{$recipe_post_data['recipe_cook_time']} {$recipe_post_data['recipe_cook_time_text']}"; ?>
        </div>
