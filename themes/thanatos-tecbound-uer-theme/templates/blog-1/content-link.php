...
<article id="post-<?php the_ID(); ?>" <?php post_class( implode(' ', $article_classes) ); ?> itemscope="itemscope" itemtype="http://schema.org/BlogPosting" itemprop="blogPost">
	<div class="post-inner">
		<!-- Post link -->
		<?php if(!empty($goza_post_options['link'])) { ?>
			<div class="post-block-link-wrap" style="background: url(<?php echo esc_attr($goza_post_thumbnail_url); ?>) no-repeat center center / cover;">
				<i class="fa fa-link"></i>
				<?php echo "{$goza_post_options['link']}"; ?>
			</div>
		<?php } ?>
